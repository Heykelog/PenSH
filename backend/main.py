...
async def export_report_to_pdf(
    ...
    try:
        ...
        generator = PentestReportGenerator(theme=template)
...
# Excel Export endpoint (POST)
@app.post("/export/xlsx/{report_id}")
async def export_report_to_xlsx_post(
    report_id: int,
    db: Session = Depends(get_db)
):
    """POST is preserved for compatibility; use the template-based exporter."""
    return await export_report_to_xlsx(report_id, db)
...
async def export_report_to_xlsx(
    ...
    """Raporu Excel formatında dışa aktar (import template header uyumlu)."""
    report = db.query(Report).filter(Report.id == report_id).first()
    if not report:
        raise HTTPException(status_code=404, detail="Rapor bulunamadı")
    ...
    template_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "xlsx_import_example.xlsx")
    )
    if not os.path.exists(template_path):
        raise HTTPException(status_code=500, detail="XLSX şablonu bulunamadı")

    template_wb = load_workbook(template_path, data_only=True)
    template_ws = template_wb.active
    headers = [cell.value for cell in template_ws[1]]
    template_wb.close()
    ...
    ws.title = "Import"
    ws.append(headers)
    header_index = {str(h).strip(): idx for idx, h in enumerate(headers) if h}
    ...
    for f in findings:
        ...
        name_val = clean_text(f.title or "")
        ip_val = "127.0.0.1"
        risk_key = f.risk_level.value if hasattr(f.risk_level, "value") else str(f.risk_level)
        risk_val = risk_map.get(str(risk_key).lower(), "")
        if risk_val != "":
            risk_val = int(risk_val)
        description_val = clean_text(f.description or "")
        if isinstance(f.created_at, datetime):
            date_val = f.created_at.strftime("%Y-%m-%d %H:%M")
        else:
            # Best-effort format to requested pattern
            try:
                date_val = datetime.fromisoformat(str(f.created_at)).strftime("%Y-%m-%d %H:%M")
            except Exception:
                date_val = ""
        solution_val = clean_text(f.solution or "")
        status_val = "OPEN"
        protocol_val = "TCP"

        if "Name (Required)" in header_index:
            row[header_index["Name (Required)"]] = name_val
        if "IP (Required)" in header_index:
            row[header_index["IP (Required)"]] = ip_val
        risk_col_idx = None
        if "Risk Level (Required)" in header_index:
            risk_col_idx = header_index["Risk Level (Required)"]
            row[risk_col_idx] = risk_val
        if "Description (Required)" in header_index:
            row[header_index["Description (Required)"]] = description_val
        if "Date(yyyy-MM-dd HH:mm)" in header_index:
            row[header_index["Date(yyyy-MM-dd HH:mm)"]] = date_val
        if "Solution" in header_index:
            row[header_index["Solution"]] = solution_val
        if "Status" in header_index:
            row[header_index["Status"]] = status_val
        if "Protocol" in header_index:
            row[header_index["Protocol"]] = protocol_val

        row = [clean_text(v) for v in row]
        ws.append(row)
        if risk_col_idx is not None:
            cell = ws.cell(row=ws.max_row, column=risk_col_idx + 1)
