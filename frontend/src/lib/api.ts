...
const PDF_TEMPLATE_KEY = 'pdfTemplate'

export const getPdfTemplate = (): string => {
  if (typeof window === 'undefined') return 'emerald'
  return localStorage.getItem(PDF_TEMPLATE_KEY) || 'emerald'
}

export const setPdfTemplate = (value: string) => {
  if (typeof window === 'undefined') return
  localStorage.setItem(PDF_TEMPLATE_KEY, value)
}
...
export const exportAPI = {
  exportToPDF: (reportId: number, includeLogo: boolean = false, template?: string) => 
    api.post(`/export/pdf/${reportId}`, {}, { 
      params: { include_logo: includeLogo, template: template ?? getPdfTemplate() },
      responseType: 'blob'
    }),
